...
const ServicesSection = () => {
  ...
  const scrollByCard = (direction) => {
    ...
    const cardWidth = firstChild ? firstChild.getBoundingClientRect().width : container.clientWidth / 3;
    const gap = 32; // approximate gap-8 in Tailwind (2rem ~ 32px)
  ...
  return (
    <section id="services" className="py-24 bg-gradient-to-b from-slate-50 to-white relative overflow-hidden">
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        {/* Carousel Row (prevents wrapping on large screens) */}
        <div className="relative max-w-7xl mx-auto">
          {/* Scrollable container */}
          <div
            ref={scrollRef}
            className="flex flex-nowrap gap-8 overflow-x-auto lg:overflow-hidden py-2"
            style={{ scrollBehavior: 'smooth' }}
          >
            {SERVICES.map((service, idx) => (
              <div
                key={idx}
                className="animate-fade-in-up flex-shrink-0 w-[85%] sm:w-[60%] md:w-[45%] lg:w-[32%]"
          ...
          <button
            ...
            aria-label="Previous"
            className="hidden lg:flex absolute left-0 top-1/2 -translate-y-1/2 bg-white/90 border border-slate-200 rounded-full p-2 shadow hover:shadow-lg hover:scale-105 transition"
          >
            <ChevronLeft size={20} />
          </button>
          <button
            ...
            aria-label="Next"
            className="hidden lg:flex absolute right-0 top-1/2 -translate-y-1/2 bg-white/90 border border-slate-200 rounded-full p-2 shadow hover:shadow-lg hover:scale-105 transition"
          >
